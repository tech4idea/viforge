...
4. 使用 `DESKTOP_BINARIES_TOKEN` checkout `YukeonWayne/pg_pgvector_binary` 到 `desktop-binaries/`。
5. 复制 `desktop-binaries/postgres/win32-x64` 到 `apps/desktop/resources/postgres/win32-x64`。
6. 设置 `VIFORGE_POSTGRES_PLATFORM_ARCH=win32-x64` 和 `VIFORGE_REQUIRE_PGVECTOR=1`，运行 `prepare:postgres` 强制校验 PostgreSQL 与 pgvector。
