...
```text
YukeonWayne/viwork-desktop-binaries
```
...
2. 选择只允许访问 `YukeonWayne/viwork-desktop-binaries`。
3. 权限只给 `Contents: Read-only`。
4. 在主仓库 `YukeonWayne/viwork` 的 `Settings -> Secrets and variables -> Actions` 新增 secret：
...
- name: Checkout desktop binaries
  ...
  with:
    repository: YukeonWayne/viwork-desktop-binaries
...
- name: Verify PostgreSQL bundle
  ...
  env:
    VIWORK_POSTGRES_PLATFORM_ARCH: ${{ matrix.platform_arch }}
...
```powershell
git clone https://github.com/YukeonWayne/viwork-desktop-binaries.git D:\deps\viwork-desktop-binaries
Remove-Item -Recurse -Force apps\desktop\resources\postgres\win32-x64 -ErrorAction SilentlyContinue
Copy-Item -Recurse D:\deps\viwork-desktop-binaries\postgres\win32-x64 apps\desktop\resources\postgres\win32-x64
