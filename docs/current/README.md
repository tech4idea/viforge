...
单机版面向快速上手：用户安装 exe 后直接打开 viwork，不需要预装 Node，也不需要手动启动浏览器、API、PostgreSQL 或向量库。Electron 安装包内置 Chromium 和 Node runtime；应用启动时由主进程拉起本地 API，API 在桌面模式下托管 `apps/web/dist`。

默认数据库策略是内置 PostgreSQL binary + pgvector，保持和 LangGraph `PostgresSaver` / `PostgresStore` 一致。高级用户可在“运行设置”里切换到外部 PostgreSQL connection string，后续 LangGraph 官方更多 adapter 可通过同一个配置入口扩展。模型提供商只要求兼容 OpenAI 协议，可配置 Base URL、API Key、文本/图片/embedding 模型和 embedding 维度。
