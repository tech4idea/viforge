...
当前实现已经从第一批数据合同/API 进入 RuntimeConfig 流程化阶段，重点覆盖后端存储/API、普通 run 生效路径、隔离 EvalRun 和前端 Harness 流程入口：

- 共享合同包含 `RuntimeConfigFlow`、`AgentSpec`、`AgentLayerConfig`、`BehaviorRuleConfig`、`AgentToolPolicy`、`ToolDescriptionConfig`、`AssertionConfig`、`EvalRunConfig`、`MemoryPolicy`、`MemoryRecord`、`KnowledgeBaseEntry`、`RetrievalPolicy`、`RunInputSnapshot`、`EvalFixture`、`EvalRun`、`HumanReviewRubric`、`AgentSpecReleaseRecord` 和 `HarnessSummary`。
- API 新增 `HarnessStore`，文件型持久化在 `WORKSPACES_ROOT/../harness`。
- 创建普通 run 时，API 会先创建 `RunInputSnapshot`，默认复制全项目轻量快照，排除 `.git`、`node_modules`、`dist`、构建缓存、日志等；snapshot 同时记录当次用户 prompt，便于后续从 snapshot 直接生成单轮 fixture。
- API 暴露 `/api/harness`、RuntimeConfigFlow、AgentLayerConfig、AgentSpec、BehaviorRuleConfig、AgentToolPolicy、ToolDescriptionConfig、MemoryPolicy、KnowledgeBaseEntry、RetrievalPolicy、PromptBlock、SkillSnapshot、WorkspaceManifest 只读列表、RunInputSnapshot、RunArtifact、EvalFixture、AssertionConfig、EvalRunConfig、EvalRun、HumanReviewRubric、release gate/release record 等 Harness 路由。
- Web 独立 Agent Harness 页面可从工具入口在新标签页打开。当前页面先展示运行配置流程列表，再按“评测案例上下文 / Agent 配置 / 评测运行与断言 / 运行、审阅与发布”组织一次配置流程。页面支持创建、克隆、归档、删除未引用 RuntimeConfigFlow 草稿，创建候选 AgentSpec，创建版本化 BehaviorRuleConfig、ToolDescriptionConfig、AssertionConfig、EvalRunConfig 和 HumanReviewRubric，从历史对话/snapshot/RunArtifact 创建 fixture，创建 Repro/Live EvalRun，并在评测后执行发布或回滚。SkillSnapshot 目前只作为运行证据和 resolved config 来源保留，不作为普通 UI 配置入口。
...
- 从 RunArtifact 生成 EvalFixture 时，系统会带上 expectedChangedFiles、expectedToolEvents、memoryFixture、knowledgeFixture、tool retention policy 和裁剪后的 toolRetentionArtifacts。断言已经从主流程中拆到独立 AssertionConfig；历史 `EvalFixture.assertions` 字段仍保留为兼容 fallback，但新 UI 会优先创建和绑定 AssertionConfig。
- EvalFixture 支持 PATCH 更新，可维护 input messages、expectedChangedFiles、expectedToolEvents、memoryFixture、knowledgeFixture、toolMocks、tags 和 tool retention。断言配置由 AssertionConfig 管理，支持文件变化、路径约束、Markdown section、工具事件、diff 限制和 WorkspaceManifest 合规检查。
...
- HarnessStore 已预留 `evalRunExecutor` 注入点。默认 executor 使用 fixture 回放；正式 app 已接入 LangGraph isolated executor，让候选 AgentSpec 可以在 fixture 工作区里真实执行，workspace tools 指向隔离目录，不污染原项目。Repro 模式下 memory/knowledge 工具会优先使用 EvalFixture 中固定的 `memoryFixture` 和 `knowledgeFixture`，避免回归比较受 live 记忆或当前知识库变化影响。isolated executor 现在会把 resolved `AgentLayerConfig` 传入 agent registry，product 级 specialist 启用列表、system/specialist `instructionOverride`、行为规则、工具描述覆盖、tool/model/memory/retrieval policy refs 都会进入实际执行路径和 prompt 摘要，不再只是记录在 EvalRun metadata 中。候选 AgentSpec 绑定的 PromptBlock 会拼入对应 system/specialist instruction，确保新增或修改的行为规则实际进入 EvalRun prompt。`resolvedAgentConfig` 也会保存 prompt block / skill / tool description / policy 的内容 hash 和来源，便于判断候选 spec 实际用了哪些配置内容。
- 普通 LangGraph run 也会读取当前 product 的 active resolved runtime config。active `BehaviorRuleConfig` 会拼入主 agent instruction，active `AgentToolPolicy` 会控制可用工具/禁用工具，active `ToolDescriptionConfig` 会覆盖工具 description、参数说明和输出说明。这样评测中的临时配置和线上运行使用同一套解析逻辑；未发布的 draft/candidate 仍只在 EvalRun 中生效。
- AgentSpec 发布 gate：active 发布默认要求至少一个 passed EvalRun；人工评分作为参考证据保存，不再阻塞发布。发布会归档同 product/agent 的旧 active，并写入 releaseRecords。API 暴露 release-gate 检查结果，紧急场景可 force，但需要显式传入。releaseRecords 会保存 force 标记和当时的 gate 快照，便于审计为什么绕过发布检查。
- Web Harness 面板已按 HumanReviewRubric 渲染评分项，可切换或创建打分模板，保存 scores、subScores 和 notes；同一面板可查看当前 AgentSpec 的评分汇总、评分历史、发布 gate 的通过/阻塞原因，并提供带浏览器确认的强制发布入口。Fixture 对比区已支持 active/candidate changedFiles 的逐行并排 diff，并把对应人工备注放在同一阅读区域。
...
- Harness 保留 `WorkspaceManifest` 数据模型和只读 API，resolved agent config 会带上当前 product 的 active manifest；程序断言可以使用 manifest canonical path、requiredSections 和 requiredDirectories，不再只依赖 product profile 的静态 artifactPaths。当前新交互不开放 WorkspaceManifest 创建或发布。
- Web 评测与断言步骤提供独立 AssertionConfig 创建入口。Fixture 侧不再作为主断言编辑入口；高级 JSON 仍用于兼容历史 fixture assertions。
...
- Active 配置预览已改成三层：默认只展示用户决策摘要、当前 agent 提示词和绑定行为规则；配置页右侧固定“Agent 上下文全貌”预览区，展示 active/candidate 及简单差异；更底层的 `AgentSpec`、`AgentLayerConfig`、MemoryPolicy、RetrievalPolicy、WorkspaceManifest、全部 specialist 和工具权限收进“高级审计信息”。配置页会记住上次选择的产品、agent、fixture、candidate、EvalRun 和 tab。
...
1. 继续打磨真实 LangGraph 隔离执行。当前已完成 executor 适配器、app 接入、隔离 workspace、Repro memory/knowledge fixture 注入、Live/Repro 模式切换，并已让 executor 与普通 run 共用 resolved runtime config：启用 specialist、system/specialist instruction override、行为规则、工具描述、工具允许/禁用 policy 和 memory/retrieval/model refs 都会实际生效。resolved config 已记录 prompt block / skill / tool description / policy 的内容 hash 和来源。独立 PromptBlock 存储已经进入 shared contracts、API 与 Harness 面板创建入口，并支持创建新版本；SkillSnapshot 相关 API 和合同保留用于证据链，但 UI 暂不作为用户编辑入口。后续重点是给 PromptBlock 做更清晰的 diff 视图和发布状态机。
2. 继续增强 EvalFixture 与 AssertionConfig 的衔接。当前已能从真实 run artifact 自动补消息、工具事件、文件 diff、memory、knowledge 和 tool retention；断言已独立为可复用 AssertionConfig。后续要把从失败运行生成断言建议、从工具事件生成断言、断言失败回跳到配置项和 diff 行整合成更流畅的分步向导。
