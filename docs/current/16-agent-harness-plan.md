...
- Web 新增独立 Agent Harness 页面，可从工具入口在新标签页打开。页面按“配置候选 / 沉淀案例 / 执行评测 / 人工审阅 / 发布运行”分 tab 展示，可查看统计、新建 draft AgentSpec、创建显式 Agent 行为规则（底层为 PromptBlock）、从历史对话/snapshot/RunArtifact 创建 fixture、创建 Repro/Live EvalRun，并在评测和人工审阅后执行发布或回滚。SkillSnapshot 目前只作为运行证据和 resolved config 来源保留，不作为普通 UI 配置入口。
...
- EvalFixture 支持 PATCH 更新，前端可直接编辑 assertions JSON，也可用表单化断言列表增删常见自动断言，覆盖 `files`、`toolEvents`、`diff.allowedPaths`、`diff.allowedChanges`、`diff` 数量限制、`manifest` 和 Markdown section 等。Fixture 也可维护 input messages、expectedChangedFiles、expectedToolEvents、memoryFixture、knowledgeFixture，并可调整 tool retention，作为“系统推荐 -> 人工确认/裁剪”的 V1 工作流。
