...
当前构建产物（`pnpm --filter @viforge/web build`）仍是单个 JS bundle：
...
1. `pnpm --filter @viforge/web typecheck` 通过
2. `pnpm --filter @viforge/web test` 通过
3. `pnpm --filter @viforge/web build` 通过，确认产出两个 JS chunk + 两个 CSS chunk
4. `pnpm --filter @viforge/api test` 通过（无前端变更影响，作为回归确认）
5. 手动验证：
   - 启动 dev server（`pnpm --filter @viforge/web dev`）
