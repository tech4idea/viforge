...
本文面向把 ViForge 单机版打成可安装程序的流程。核心原则：先准备好对应平台的 PostgreSQL + pgvector 资源包，再运行 Electron Builder 生成安装包。
...
6. API 和内置 PostgreSQL 启动期间会显示“ViForge 正在打开”占位窗口，任务栏可见。
7. 重复点击桌面图标不会启动第二个实例，只会聚焦当前启动窗口或主窗口。
8. 点击窗口关闭按钮会弹出确认框，可选择“仅关闭窗口”或“完全退出”。
