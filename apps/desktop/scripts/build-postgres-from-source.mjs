...
const postgresVersion = process.env.VIFORGE_POSTGRES_SOURCE_VERSION || '16.10';
const workRoot = path.resolve(process.env.VIFORGE_POSTGRES_BUILD_ROOT || path.join(os.tmpdir(), 'viforge-postgres-build'));
const sourceRoot = path.resolve(process.env.VIFORGE_POSTGRES_SOURCE_DIR || path.join(workRoot, `postgresql-${postgresVersion}`));
const installRoot = path.resolve(process.env.VIFORGE_POSTGRES_INSTALL_ROOT || path.join(workRoot, `install-${platformArch}`));
const targetRoot = path.join(desktopRoot, 'resources', 'postgres', platformArch);

if (process.platform === 'win32') {
  throw new Error('Source build automation currently supports Linux/macOS hosts. For Windows, build PostgreSQL in a suitable CI image and set VIFORGE_POSTGRES_BUNDLE_SOURCE.');
}
...
if (!(await exists(path.join(sourceRoot, 'configure')))) {
  throw new Error([
    ...
    'or set VIFORGE_POSTGRES_SOURCE_DIR to an extracted PostgreSQL source tree.',
  ].join(' '));
}
