...
const pgvectorVersion = process.env.VIWORK_PGVECTOR_SOURCE_VERSION || '0.8.0';
const workRoot = path.resolve(process.env.VIWORK_POSTGRES_BUILD_ROOT || path.join(os.tmpdir(), 'viwork-postgres-build'));
const sourceRoot = path.resolve(process.env.VIWORK_PGVECTOR_SOURCE_DIR || path.join(workRoot, `pgvector-${pgvectorVersion}`));
const postgresRoot = path.resolve(process.env.VIWORK_POSTGRES_INSTALL_ROOT || path.join(desktopRoot, 'resources', 'postgres', platformArch));
...
if (!(await exists(path.join(sourceRoot, 'Makefile')))) {
  throw new Error([
    ...
    'or set VIWORK_PGVECTOR_SOURCE_DIR to an extracted pgvector source tree.',
  ].join(' '));
}

const pgConfig = path.join(postgresRoot, 'bin', 'pg_config');
if (!(await exists(pgConfig))) {
  throw new Error(`PostgreSQL pg_config was not found at ${pgConfig}. Build PostgreSQL first with pnpm --filter @viwork/desktop build:postgres.`);
}
