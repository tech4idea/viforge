...
import { resolveProductProfile } from '@viforge/shared';
...
const FALLBACK_WORKSPACES_ROOT = path.join(
  ...
  '.viforge',
  'data',
  resolveProductProfile(process.env.VIFORGE_PRODUCT).id,
  'workspaces',
);

export const PRODUCT_PROFILE = resolveProductProfile(process.env.VIFORGE_PRODUCT);
...
export const AIGC_HUB_BASE_URL = process.env.VIFORGE_AIGC_HUB_BASE_URL ?? '';

export const AIGC_HUB_API_KEY = process.env.VIFORGE_AIGC_HUB_API_KEY ?? '';

export const AIGC_HUB_APP_ID = process.env.VIFORGE_AIGC_HUB_APP_ID || `viforge-${PRODUCT_PROFILE.id}`;
