...
const originalToken = process.env.VIFORGE_DESKTOP_ACCESS_TOKEN;

afterEach(() => {
  if (originalToken === undefined) {
    delete process.env.VIFORGE_DESKTOP_ACCESS_TOKEN;
  } else {
    process.env.VIFORGE_DESKTOP_ACCESS_TOKEN = originalToken;
  }
});

describe('desktop access guard', () => {
  it('requires the desktop token when enabled and stores it in an HttpOnly cookie', async () => {
    process.env.VIFORGE_DESKTOP_ACCESS_TOKEN = 'desktop-secret';
    const app = new Hono();
    ...
    expect(first.headers.get('set-cookie')).toContain('viforge_desktop_token=desktop-secret');

    const second = await app.request('/api/ping', {
      headers: { cookie: 'viforge_desktop_token=desktop-secret' },
    });
