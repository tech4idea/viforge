...
import type { ProductProfile } from '@viforge/shared';

const SHARED_SRC_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..', 'packages', 'shared', 'src');
const PRODUCT_PROMPTS_ROOT = process.env.VIFORGE_PRODUCT_PROMPTS_ROOT || path.join(SHARED_SRC_ROOT, 'product-profiles');
